...
class UserController extends Controller
{
    ...
    public function check_create($name, $email)
    {
        ...
        $user =User::where('name', $name)->orWhere('email', $email)
            ->firstOrCreate(['name'=>$name,'email'=>$email,'password'=>'test123']);
    ...
    public function check_update($name, $email)
    {
        ...
        //   if not found, create a user with $name, $email and random password
        $user = User::where('name', $name)->updateOrCreate(['email' => $email],['name'=>$name,'email' => $email,'password'=>'test123']); // updated or created user
